...
class Playlist():
    ...
    def track_next(self):
        if self.__playlist.currentIndex() == self.__playlist.mediaCount()-1:
            self.__playlist.setCurrentIndex(0)
        else:
            self.__playlist.next()

    def track_previous(self):
        if self.__playlist.currentIndex() == 0:
            self.__playlist.setCurrentIndex(self.__playlist.mediaCount()-1)
        else:
...
if __name__ == '__main__':
    playlist = Playlist()
    track = QUrl('d:/музыка/Kiss Discography/03.Compilations/1988 '
                 '- Chikara [1988 Polystar P30R-20008 Japan]/03 - Love Gun.mp3')
    playlist.set_audio(track)
    track = QUrl('d:/музыка/Kiss Discography/03.Compilations/1988 - Chikara [1988 Polystar P30R-20008 Japan]/04 '
                 '- I Was Made For Lovin You (Long Version).mp3')
    playlist.set_audio(track)
